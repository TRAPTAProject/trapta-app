...
public class  MainActivity extends Activity implements OnItemClickListener, Watchdog  {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (matchId>-1 && archerList.size()>1) {
			if (matchMode==0) match = new MatchSet(matchId, pref.getBoolean("trispot", false), StaticParam.matchVolleyMax, archerList.get(0), archerList.get(1));
			else match = new MatchPoint(matchId, pref.getBoolean("trispot", false), StaticParam.matchVolleyMax, archerList.get(0), archerList.get(1)); 
		...
		if (wifiManager!=null) {
            ...
            // WIFI lock
            if (wifiLock==null) wifiLock = wifiManager.createWifiLock("TRAPTAWIFI");
            if (wifiLock!=null) {
			...
			QuestionDialog questionDialog = new QuestionDialog(this, "Impossible d'accéder à la fonction WIFI de l'appareil. Veuillez activer le WIFI et redémarrer.",
					"OK", null);
		...
		if (ipAddress==0) {
			QuestionDialog questionDialog = new QuestionDialog(this, "L'appareil n'est connecté à aucun réseau WIFI. La synchronisation avec le serveur TRAPTA ne pourra se faire que si la connexion est établie.",
					"OK", null);
			questionDialog.show();
	...
	private void updateListAdapter() {
		if (StaticParam.arrowCount==6) {
			...
			if (StaticParam.colorInverted) layoutId = R.layout.cellview_volley6_inverted;
			if (match==null) listAdapter = new Volley6ListAdapter(this, layoutId);
			else listAdapter = new Volley6MatchAdapter(this,  layoutId, match);
		}
		else {
			int layoutId = R.layout.cellview_volley3;
			if (StaticParam.colorInverted) layoutId = R.layout.cellview_volley3_inverted;
			if (match==null) listAdapter = new Volley3ListAdapter(this, layoutId);
			else listAdapter = new Volley3MatchAdapter(this,  layoutId, match);
		}
	...
	private void checkColors() {
		...
		if (StaticParam.colorInverted) targetView.setTextColor(Color.BLACK);
		updateListAdapter();
	...
	public void showNetworkFlag(boolean value) {
		...
		runOnUiThread(new Runnable() {
            ...
            @Override
            public void run() {
                if (showValue) networkFlag.setVisibility(View.VISIBLE);
                else networkFlag.setVisibility(View.INVISIBLE);
            }
        });
	}
	...
	private void setCurrentHeat(int run) {
		if (run<0 || run>=HEAT_COUNT) return;
		...
		if (match!=null) runView.setText("MATCH");
	...
	private void setCurrentArcher(int index) {
		if (index>=archerList.size()) return;
		currentArcher = index;
		...
		for (int i=0; i<archerButton.length; i++) {
			if (i==buttonIndex) {
				if (StaticParam.colorInverted) archerButton[i].setBackgroundResource(R.drawable.buttonb_inverted);
				else archerButton[i].setBackgroundResource(R.drawable.buttonb);
			} 
			else {
				if (StaticParam.colorInverted) archerButton[i].setBackgroundResource(R.drawable.buttona_inverted);
				else archerButton[i].setBackgroundResource(R.drawable.buttona);
			}
		}
        ...
        switch (index) {
            ...
            case 3: {
                ...
                if (!StaticParam.standAloneMode) {
                    if (match!=null) TRAPTASocket.getInstance().sendMatch(match.getMatchId(), match);
                    else {
                        Archer archer = archerList.get(currentArcher);
                        TRAPTASocket.getInstance().sendHeat(archer.getId(), currentHeat, archer.getHeatList().get(currentHeat));
                    }
                }
                break;
            }
				...
				protected void onPostExecute(JSONObject jsonObject) {
					...
					try {
						...
						if (jsonObject.has("heatVolleyCount")) {
							...
							if (volleyCount>0 && volleyCount<11) StaticParam.heatVolleyCount = volleyCount;
							SharedPreferences.Editor editor = pref.edit();
						...
						if (jsonObject.has("matchVolleyMax")) {
							...
							if (matchVolleyCount>0 && matchVolleyCount<11) StaticParam.matchVolleyMax = matchVolleyCount;
							SharedPreferences.Editor editor = pref.edit();
						...
						if (jsonObject.has("arrowCount")) {
							...
							if (arrowCount==3 || arrowCount==6) StaticParam.arrowCount = arrowCount;
							SharedPreferences.Editor editor = pref.edit();
						...
						if (jsonObject.has("roundCount")) {
							...
							if (roundCount==2 || roundCount==4) StaticParam.roundCount = roundCount;
							SharedPreferences.Editor editor = pref.edit();
						...
						if ((response!=0 && response!=1) || jsonObject.isNull("data")){
							...
							if (response==1) intent.putExtra("match", true);
							else intent.putExtra("match", false);
	...
	private void initArcherListWithResults(JSONObject jsonArrowList) {
		...
		for (Archer archer:archerList) {
			try {
				...
				if (array!=null) archer.setJsonArrowList(array);
		...
		updateListAdapter();
		setCurrentHeat(0);
		if (guideEnabled) startGuideActivity();
	
	}
	
	private void configureArcherButton() {
		
		if (match==null) scoreMatchText.setVisibility(View.GONE);
		else scoreMatchText.setVisibility(View.VISIBLE);
		for (int index=0; index<4; index++) archerButton[index].setVisibility(View.GONE);
	...
	private void loadMatchResults(int targetId) throws JSONException {
		...
		if (json==null) throw new JSONException("Target "+targetId+" not found in JSON Target list");
		int matchId = json.getInt("matchId");
		boolean matchTrispot = json.getBoolean("trispot");
		int matchMode = 0;
		if (json.has("matchmode")) matchMode = json.getInt("matchmode");
		...
		if (matchMode==0) match = new MatchSet(matchId, matchTrispot,StaticParam.matchVolleyMax, archerA, archerB);
		else match = new MatchPoint(matchId, matchTrispot,StaticParam.matchVolleyMax, archerA, archerB);
		SharedPreferences.Editor editor = pref.edit();
	...
	private void loadArcherResults(int targetId) throws JSONException {
		...
		if ((json==null || (json.length()==0))) throw new JSONException("Target "+targetId+" not found in JSON Target list");
	...
	private void updateMatchScore() { 
		if (match!=null) {  
			...
			String strA = "?";
			String strB = "?";
			if (scoreA>-1) strA = ""+scoreA;
			if (scoreB>-1) strB = ""+scoreB;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode==CODE_GUIDE_ACTIVITY && resultCode==RESULT_OK) startVisualGuide1();
		...
		if (requestCode==CODE_VOLLEY_ACTIVITY && resultCode==RESULT_OK) {
			...
			if (!StaticParam.standAloneMode) {
				if (match!=null) TRAPTASocket.getInstance().sendMatch(match.getMatchId(), match);
				else TRAPTASocket.getInstance().sendHeat(id, currentHeat, archer.getHeatList().get(currentHeat));
			}
		...
		if (requestCode==CODE_WELCOME_ACTIVITY && resultCode==RESULT_OK) {
			...
			if ((standAloneMode) || (jsonTargetList==null)) {
				...
				try {
					if (isMatch) loadMatchResults(targetId);
					else loadArcherResults(targetId);
	...
	private void startVolleyActivity(int volleyIndex, int[] array) {
		...
		if (guideEnabled) intent.putExtra("play", true);
		else intent.putExtra("play", false);
		goneToVolley = true;
	...
	private void prepareVolleyActivity(List<Volley> volleyList) {
		...
		for (int i=0; i<StaticParam.arrowCount; i++) array[i] = -1;
	...
	public void onItemClick(AdapterView adapterView, View view, int position, long id) {
		 ...
		 if (match==null) {
			 ...
			 if (position==StaticParam.heatVolleyCount) return;
			 
			 // ask for confirmation if same archer
			 if (currentArcher==latestArcherUpdate && archerList.size()>1 && !StaticParam.standAloneMode) {
				 QuestionDialog dialog = new QuestionDialog(this, "Voulez-vous vraiment entrer une nouvelle volée pour ce même archer ?", "Annuler", "Oui,\nencore une volée");
				 dialog.setOnButtonClickListener(new QuestionButtonListener() {
			 ...
			 if (position==StaticParam.matchVolleyMax && match.needArrowBreak()) whoWins();
		...
		public void run() {
			...
			switch (step) {
				case 0: {
						...
						anim = new TranslateAnimation(StaticParam.screenWidth/2-targetX,0,(int)(StaticParam.screenHeight*0.7)-targetY,0);
				...
				case 4: {
						...
						anim = new TranslateAnimation(handLayout.leftMargin-targetX,0,handLayout.topMargin-targetY,0);
						handLayout.leftMargin = targetX;
						handLayout.topMargin = targetY;
		...
		public void run() {
			...
			switch (step) {
				...
				case 0: {
					...
					handLayout.leftMargin = targetX;
					handLayout.topMargin = targetY;
					anim = new TranslateAnimation(StaticParam.screenWidth/2-targetX,0,StaticParam.screenHeight/2-targetY,0);
					anim.setDuration(1000);
					handImage.setAnimation(anim);
					anim.start();
